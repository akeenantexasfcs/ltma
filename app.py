...
def income_statement():
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            ...
            labels = ["Revenue", "Cost of Goods Sold", "Gross Profit", "Operating Expenses", "Net Income"]
            selections = []
            ...
            if 'Account' not in df.columns:
                ...
                if st.button("Generate Excel with Lookup Results", key="generate_excel_lookup_results_tab3_is"):
                    ...
                    def lookup_ciq(mnemonic):
                        if mnemonic == 'Human Intervention Required':
                            return 'CIQ IQ Required'
                        ciq_value = income_statement_lookup_df.loc[income_statement_lookup_df['Mnemonic'] == mnemonic, 'CIQ']
                        if ciq_value.empty:
                            return 'CIQ IQ Required'
                        return ciq_value.values[0]
                ...
                if st.button("Update Data Dictionary with Manual Mappings", key="update_data_dictionary_tab3_is"):
                    ...
                    for idx, row in df.iterrows():
                        ...
                        if manual_selection == 'REMOVE ROW':
                            continue
                        ciq_value = income_statement_lookup_df.loc[income_statement_lookup_df['Mnemonic'] == final_mnemonic, 'CIQ'].values[0] if not income_statement_lookup_df.loc[income_statement_lookup_df['Mnemonic'] == final_mnemonic, 'CIQ'].empty else 'CIQ IQ Required'
                        
                        if manual_selection not in ['REMOVE ROW', '']:
                            if row['Account'] not in income_statement_lookup_df['Account'].values:
                                new_entries.append({'Account': row['Account'], 'Mnemonic': final_mnemonic, 'CIQ': ciq_value, 'Label': row['Label']})
                            else:
                                income_statement_lookup_df.loc[income_statement_lookup_df['Account'] == row['Account'], 'Mnemonic'] = final_mnemonic
                                income_statement_lookup_df.loc[income_statement_lookup_df['Account'] == row['Account'], 'Label'] = row['Label']
                                income_statement_lookup_df.loc[income_statement_lookup_df['Account'] == row['Account'], 'CIQ'] = ciq_value
                    if new_entries:
