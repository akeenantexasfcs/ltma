...
def get_ai_suggested_mapping_CF(label, account, cash_flow_lookup_df):
    ...
    And the following cash flow statement lookup data:
    {cash_flow_lookup_df.to_string()}

    What is the most appropriate Mnemonic mapping for this account based on Label and Account combination? Please provide only the value from the 'Mnemonic' column in the Cash Flow Statement Data Dictionary data frame based on Label and Account combination, without any explanation. The determination should be based on business logic first then similarity. Ensure that the suggested Mnemonic is appropriate for the given Label e.g., don't suggest an Operating Activities Mnemonic for a Financing Activities Label."""
...
def balance_sheet_BS():
    ...
    with tab4:
        ...
        uploaded_dict_file = st.file_uploader("Upload a new Data Dictionary CSV", type=['xlsx'], key='dict_uploader_tab4_bs')
        if uploaded_dict_file is not None:
            new_lookup_df = pd.read_excel(uploaded_dict_file)
            balance_sheet_lookup_df = new_lookup_df  # Overwrite the entire DataFrame
            save_lookup_table(balance_sheet_lookup_df, balance_sheet_data_dictionary_file)
