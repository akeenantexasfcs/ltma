...
# Optimized embedding function with batch processing
...
# Optimized function for AI suggested mapping using batch processing
def get_ai_suggested_mapping_BS(label, account, balance_sheet_lookup_df, nearby_rows):
    ...
    account_embedding = get_embeddings([f"{label} {account}"])[0]
    similarities = balance_sheet_lookup_df.apply(lambda row: cosine_similarity(account_embedding, get_embeddings([f"{row['Label']} {row['Account']}"])[0]), axis=1)
...
# Efficient file processing function
def process_file_concurrently(file):
...
# Other functions such as balance_sheet_BS, cash_flow_statement_CF, income_statement, populate_ciq_template_pt, and extras_tab remain the same but optimized where possible
    ...
    with tab2:
        ...
        if uploaded_files:
            with ThreadPoolExecutor() as executor:
                dfs = list(executor.map(process_file_concurrently, uploaded_files))

            combined_df = pd.concat([df[list(df.keys())[0]] for df in dfs], ignore_index=True)
            st.dataframe(combined_df)

            aggregated_table = combined_df.groupby(['Label', 'Account'], as_index=False).sum()
            ...
            zero_rows = (aggregated_table.iloc[:, 2:] == 0).all(axis=1)
        ...
        if uploaded_excel is not None:
            ...
            if 'Account' not in df.columns:
                ...
                if st.button("Generate Excel with Lookup Results", key="generate_excel_lookup_results_tab3_bs"):
                    ...
                    date_cols = [col for col in final_output_df.columns if col not in ['Label', 'Account', final_mnemonic_col, 'Mnemonic', 'Manual Selection']]
                    df_grouped = final_output_df.groupby([final_mnemonic_col, 'Label']).sum(numeric_only=True).reset_index()
                    ...
                    combined_df = df_pivot if combined_df.empty else combined_df.join(df_pivot, how='outer')
...
def cash_flow_statement_CF():
    ...
    with tab3:
        ...
        uploaded_excel_is = st.file_uploader("Upload your Excel file for Mnemonic Mapping", type=['xlsx'], key='excel_uploader_tab3_cfs')
        ...
        selected_currency_is = st.selectbox("Select Currency", currency_options_is, key='currency_selection_tab3_cfs')
        selected_magnitude_is = st.selectbox("Select Magnitude", magnitude_options_is, key='magnitude_selection_tab3_cfs')
        ...
        if uploaded_excel_is is not None:
            ...
            if 'Account' not in df_is.columns:
                ...
                for idx, row in df_is.iterrows():
                    ...
                    manual_selection_options = cash_flow_lookup_df['Mnemonic'].unique()
                    manual_selection_is = st.selectbox(
                        f"Select category for '{account_value}'",
                        options=[''] + list(manual_selection_options),
                ...
                if st.button("Generate Excel with Lookup Results", key="generate_excel_lookup_results_tab3_is"):
                    ...
                    combined_df_is = pd.DataFrame()
                    final_mnemonic_col = 'Final Mnemonic Selection'
                    date_cols = [col for col in final_output_df_is.columns if col not in ['Account', final_mnemonic_col, 'Mnemonic', 'Manual Selection', 'Sort Index']]
                    df_grouped = final_output_df_is.groupby([final_mnemonic_col]).sum(numeric_only=True).reset_index()
                    ...
                    combined_df_is = df_pivot if combined_df_is.empty else combined_df_is.join(df_pivot, how='outer')
                    combined_df_is.reset_index(inplace=True)

                    combined_df_is = combined_df_is.sort_values(by=['Final Mnemonic Selection'])

                    def lookup_ciq_is(mnemonic):
                        if mnemonic == 'Human Intervention Required':
                            return 'CIQ ID Required'
                        ciq_value_is = cash_flow_lookup_df.loc[cash_flow_lookup_df['Mnemonic'] == mnemonic, 'CIQ']
                    ...
                    with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
                        ...
                        cover_df_is = pd.DataFrame({
                            'Selection': ['Currency', 'Magnitude', 'Company Name'] + list(statement_dates.keys()),
                            'Value': [selected_currency_is, selected_magnitude_is, company_name_cfs] + list(statement_dates.values())
                        })
                ...
                if st.button("Update Data Dictionary with Manual Mappings", key="update_data_dictionary_tab3_is"):
                    ...
                    for idx, row in df_is.iterrows():
                        ...
                        if manual_selection_is == 'REMOVE ROW':
                            continue
                        ciq_value_is = cash_flow_lookup_df.loc[cash_flow_lookup_df['Mnemonic'] == final_mnemonic_is, 'CIQ'].values[0] if not cash_flow_lookup_df.loc[cash_flow_lookup_df['Mnemonic'] == final_mnemonic_is, 'CIQ'].empty else 'CIQ ID Required'

                        if manual_selection_is not in ['REMOVE ROW', '']:
                            if row['Account'] not in cash_flow_lookup_df['Account'].values:
                                new_entries.append({'Account': row['Account'], 'Mnemonic': final_mnemonic_is, 'CIQ': ciq_value_is})
                            else:
                                cash_flow_lookup_df.loc[cash_flow_lookup_df['Account'] == row['Account'], 'Mnemonic'] = final_mnemonic_is
                                cash_flow_lookup_df.loc[cash_flow_lookup_df['Account'] == row['Account'], 'CIQ'] = ciq_value_is
                    if new_entries:
    ...
    with tab4:
        ...
        if uploaded_dict_file_is is not None:
            ...
            cash_flow_lookup_df = new_lookup_df_is
            save_lookup_table(cash_flow_lookup_df, cash_flow_data_dictionary_file)
            st.success("Data Dictionary uploaded and updated successfully!")

        st.dataframe(cash_flow_lookup_df)

        remove_indices_is = st.multiselect("Select rows to remove", cash_flow_lookup_df.index, key='remove_indices_tab4_is')
        rows_removed_is = False
        if st.button("Remove Selected Rows", key="remove_selected_rows_tab4_is"):
            cash_flow_lookup_df = cash_flow_lookup_df.drop(remove_indices_is).reset_index(drop=True)
            save_lookup_table(cash_flow_lookup_df, cash_flow_data_dictionary_file)
            rows_removed_is = True
            st.success("Selected rows removed successfully!")
            st.dataframe(cash_flow_lookup_df)

        st.subheader("Download Data Dictionary")
        if rows_removed_is:
            download_label_is = "Download Updated Data Dictionary"
        else:
            download_label_is = "Download Data Dictionary"
        excel_file = io.BytesIO()
...
def income_statement():
    ...
    with tab1:
        ...
        if uploaded_file is not None:
            ...
            labels = ["Operating Revenue", "Non-Operating Revenue", "Operating Expenses",
                      "Non-Operating Expenses", "Other Income", "Net Income"]
            selections = []

            for label in labels:
                st.subheader(f"Setting bounds for {label}")
                options = [''] + get_unique_options(all_tables[column_a].dropna())
                start_label = st.selectbox(f"Start Label for {label}", options, key=f"start_{label}_is")
                end_label = st.selectbox(f"End Label for {label}", options, key=f"end_{label}_is")
                selections.append((label, start_label, end_label))
            ...
            if st.button("Preview Setting Bounds ONLY", key="preview_setting_bounds_is"):
                preview_table = update_labels(all_tables.copy())
                st.subheader("Preview of Setting Bounds")
                st.dataframe(preview_table)
            ...
            selected_value = st.radio("Select conversion value", ["Actuals", "Thousands", "Millions", "Billions"], index=0, key="conversion_value_is")
    ...
    with tab2:
        ...
        if uploaded_files:
            ...
            rows_removed = False
            if st.button("Remove Highlighted Rows", key="remove_highlighted_rows_is"):
                aggregated_table = aggregated_table.drop(zero_rows_indices).reset_index(drop=True)
                rows_removed = True
                st.success("Highlighted rows removed successfully")
                st.dataframe(aggregated_table)
            ...
            with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
                aggregated_table.to_excel(writer, sheet_name='Aggregated Data', index=False)
            excel_file.seek(0)
            st.download_button(download_label, excel_file, "Aggregate_My_Data_Income_Statement.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    with tab3:
        st.subheader("Mappings and Data Consolidation")

        uploaded_excel = st.file_uploader("Upload your Excel file for Mnemonic Mapping", type=['xlsx'], key='excel_uploader_tab3_is')

        currency_options = ["U.S. Dollar", "Euro", "British Pound Sterling", "Japanese Yen"]
        magnitude_options = ["Actuals", "Thousands", "Millions", "Billions", "Trillions"]

        
        selected_currency = st.selectbox("Select Currency", currency_options, key='currency_selection_tab3_is')
        selected_magnitude = st.selectbox("Select Magnitude", magnitude_options, key='magnitude_selection_tab3_is')
            ...
            if 'Account' not in df.columns:
                ...
                if st.button("Generate AI Recommendations", key="generate_ai_recommendations_is"):
                    for idx, row in df.iterrows():
                        if row['Mnemonic'] == 'Human Intervention Required':
                            account_value = row['Account']
                            nearby_rows = df.iloc[max(0, idx-2):min(len(df), idx+3)][['Account']].to_string()
                ...
                st.dataframe(df[['Account', 'Mnemonic', 'Manual Selection', 'Sort Index']])
                ...
                if st.button("Generate Excel with Lookup Results", key="generate_excel_lookup_results_tab3_is"):
                    ...
                    final_output_df_is = df[df['Final Mnemonic Selection'].str.strip() != 'REMOVE ROW'].copy()
                    ...
                    excel_file = io.BytesIO()
                    with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
                        combined_df_is.to_excel(writer, sheet_name='Standardized - Income Statement', index=False)
                        as_presented_df_is.to_excel(writer, sheet_name='As Presented - Income Statement', index=False)
                        cover_df_is = pd.DataFrame({
                            'Selection': ['Currency', 'Magnitude', 'Company Name'] + list(statement_dates.keys()),
                            'Value': [selected_currency, selected_magnitude, company_name_is] + list(statement_dates.values())
                        })
                        cover_df_is.to_excel(writer, sheet_name='Cover', index=False)
                    excel_file.seek(0)
                    st.download_button("Download Excel", excel_file, "Mappings_and_Data_Consolidation_Income_Statement.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                ...
                if st.button("Update Data Dictionary with Manual Mappings", key="update_data_dictionary_tab3_is"):
                    ...
                    if new_entries:
                        income_statement_lookup_df = pd.concat([income_statement_lookup_df, pd.DataFrame(new_entries)], ignore_index=True)
    ...
    with tab4:
        ...
        if uploaded_dict_file_is is not None:
            new_lookup_df_is = pd.read_excel(uploaded_dict_file_is)
            income_statement_lookup_df = new_lookup_df_is
            save_lookup_table(income_statement_lookup_df, income_statement_data_dictionary_file)
            st.success("Data Dictionary uploaded and updated successfully!")
        ...
        excel_file.seek(0)
        st.download_button(download_label_is, excel_file, "income_statement_data_dictionary.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
