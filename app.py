...
def get_embedding(text):
    return model.encode(text).tolist()  # Ensure it's serializable
                    ...
                    async def generate_ai_recommendations():
                        for idx, row in df.iterrows():
                            if row['Mnemonic'] == 'Human Intervention Required':
                                ...
                                ai_suggested_mnemonic = await get_ai_suggested_mapping_BS(label_value, account_value, balance_sheet_lookup_df_json, nearby_rows)
